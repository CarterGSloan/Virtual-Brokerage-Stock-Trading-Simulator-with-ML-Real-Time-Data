    ...
    while True:
        ...
        try:
            ...
            if "rolling_auc" in res.metrics:
                print(Fore.CYAN + f"Rolling AUC (context): {res.metrics['rolling_auc']:.3f}" + Style.RESET_ALL)
                ...
                while True:
                    ...
                    if portfolio:
                        ...
                        for sh in portfolio:
                            ...
                            total_val += value
                            print(Fore.CYAN + f"{sh.symbol:<10}{sh.quantity:>10}{sh.avg_price:>12.2f}{curr_price:>12.2f}{value:>15.2f}" + Style.RESET_ALL)
                        print(Fore.CYAN + f"\nTotal Portfolio Value: ${total_val:,.2f}" + Style.RESET_ALL)
                    else:
                        print(Fore.YELLOW + "(Portfolio is empty)" + Style.RESET_ALL)
                    
                    print()
                    action = input(Fore.GREEN + "➤ Command [buy <SYM> <QTY> / sell <SYM> <QTY> / back]: " + Style.RESET_ALL).strip().lower()
                    if not action:
                        continue
                    if action in ("back", "b"):
                        break
